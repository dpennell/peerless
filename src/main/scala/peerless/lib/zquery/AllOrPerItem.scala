...
object AllOrPerItem {
  ...
  def make[R <: Logging, E, Req <: Request[E, Any], Item](
      definition: AllOrPerItem[R, E, Req, Item]
    )(implicit reqTag: ClassTag[Req]
    ): DataSource[R, Req] =
    DataSource.Batched.make(definition.name) { (requests: Chunk[Req]) =>
      log.locally(Name(definition.name :: Nil)) {
        ...
        val baseMap: ZIO[R, E, CompletedRequestMap] =
          if (containsAll)
            for {
              ...
              foldResult <-
                definition
                  .getAll()
                  .foldM((CompletedRequestMap.empty, Set.empty[Item])) {
                    case ((resultMap, all), item) =>
                      for {
